...
export async function bidirectionalDijkstraAlgo(start, end) {
  ...
  while (unvisitedFromStart.length > 0 && unvisitedFromEnd.length > 0) {
    ...
    let neighboursFromStart = currentFromStart.neighbours,
      neighboursFromEnd = currentFromEnd.neighbours;

    for (let i = 0; i < neighboursFromStart.length; i++) {
      let neighbour = neighboursFromStart[i];
      if (
        ...
        await bidirectionalPathTracing(neighbour, currentFromStart, start, end);
        return;
    ...
    for (let i = 0; i < neighboursFromEnd.length; i++) {
      let neighbour = neighboursFromEnd[i];
      if (
